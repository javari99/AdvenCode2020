...
questionaryArray = inputString.split("\n\n")
solutionSum = 0

for group in questionaryArray:

    members = group.split("\n")
    questionSet = set(list(string.ascii_lowercase))

    for member in members:
        memberSet = set([])

        for character in member:
            memberSet.add(character)
        
        questionSet = questionSet.intersection(memberSet)

    solutionSum += len(questionSet)
