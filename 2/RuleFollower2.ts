let challengeInput = ["6-9 z: qzzzzxzzfzzzz",
"2-4 s: xgmsdts",
"6-7 k: khqpsvk",
"1-3 k: knkqh",
"4-11 z: lwnzgzlmpvz",
...
let totalTruths : number = 0;

for (const phrase of challengeInput) {
    let splitted : string[] = phrase.split(':');
    let rules : string = splitted[0];
    let str : string = splitted[1].trim();
    let splittedRules : string[] = rules.split(' ');
    let letter : string = splittedRules[1];
    let splittedNumbers : string[] = splittedRules[0].split('-');
    let initial : number = parseInt(splittedNumbers[0]);
    let final : number = parseInt(splittedNumbers[1]);
    
    if ( str[initial-1] === letter || str[final-1] === letter){
        if(str[initial-1] !== str[final-1]){
            totalTruths++;
        }
    }
}
